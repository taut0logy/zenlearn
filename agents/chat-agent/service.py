...
from .schemas import (
    ...
    MessageResponse, MessageRole, ChatListResponse
)
...
class ChatService:
    ...
    async def list_chats(
        ...
        chat_responses = []
        for chat in chats:
            msg_count_query = select(func.count(Message.id)).where(Message.chat_id == chat.id)
    ...
    async def send_message(
        ...
        response_text = await self.agent.invoke(
            user_id=str(user_id),
            chat_id=str(chat_id),
            messages=[{"role": m.role.value, "content": m.content} for m in context_messages[1:]],  # Skip system
    ...
    async def send_message_stream(
        ...
        async for event in self.agent.stream(
            user_id=str(user_id),
            chat_id=str(chat_id),
            messages=[{"role": m.role.value, "content": m.content} for m in context_messages[1:]],
            system_prompt=context.get_system_prompt()
        ):
            yield event
